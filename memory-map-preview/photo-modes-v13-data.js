...
function destinationPoint(coord,bearing,distance){const R=6378137,b=rad(bearing),lat1=rad(coord[1]),lon1=rad(coord[0]),d=distance/R;const lat2=Math.asin(Math.sin(lat1)*Math.cos(d)+Math.cos(lat1)*Math.sin(d)*Math.cos(b));const lon2=lon1+Math.atan2(Math.sin(b)*Math.sin(d)*Math.cos(lat1),Math.cos(d)-Math.sin(lat1)*Math.sin(lat2));return[lon2*180/Math.PI,lat2*180/Math.PI]}
function midpoint(a,b){return[(a[0]+b[0])/2,(a[1]+b[1])/2]}
for(const cluster of Object.values(CLUSTERS))for(const obs of cluster.observations){obs.cluster=cluster.id;obs.subject=obs.subject||destinationPoint(obs.coord,obs.heading,obs.distance||22);obs.scene={center:midpoint(obs.coord,obs.subject),zoom:obs.sceneZoom||18.35,pitch:70,bearing:(obs.heading+342)%360};obs.pov={center:obs.subject,zoom:obs.povZoom||19.35,pitch:obs.pitch||88,bearing:obs.heading,elevation:obs.elevation||12}}
export const ALL_OBS=CLUSTER_ORDER.flatMap(id=>CLUSTERS[id].observations);
function clusterCenter(cluster){const n=cluster.observations.length;return[cluster.observations.reduce((s,o)=>s+o.coord[0],0)/n,cluster.observations.reduce((s,o)=>s+o.coord[1],0)/n]}
for(const c of Object.values(CLUSTERS))c.center=clusterCenter(c);
