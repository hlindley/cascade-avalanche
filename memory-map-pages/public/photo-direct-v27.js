...
// Preserve the established World → Cluster → POV state machine and the
// gradual bearing change during the high-altitude lift.
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      if (!(image.naturalWidth > 0 && image.naturalHeight > 0)) {
        reject(new Error(`JPEG decoded with invalid dimensions: ${src}`));
        return;
      }
      resolve(image);
    };
...
function hidePhoto(updateCopy = false) {
  ...
  setTimeout(() => {
    if (!photoVisible) ctx.clearRect(0, 0, canvas.width, canvas.height);
  }, 520);
  syncUi();
}
...
function replaceWithFallback(img) {
  if (!img?.parentNode || img.dataset.failed === 'true') return;
  img.dataset.failed = 'true';
  const fallback = document.createElement('span');
  fallback.className = 'missingPhoto';
...
function ensureMarkers() {
  if (!capturedMap || markerEntries.size) return;
  if (!capturedMap.isStyleLoaded?.()) return;

  // DOM photo cards replace the legacy MapLibre cluster-label layer.
  if (capturedMap.getLayer('cluster-labels')) {
    capturedMap.setLayoutProperty('cluster-labels', 'visibility', 'none');
  }
...
new MutationObserver(syncUi).observe(document.body, {
  subtree: true,
  childList: true,
  characterData: true,
  attributes: true,
