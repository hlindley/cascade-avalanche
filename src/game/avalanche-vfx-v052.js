...
CascadeScene.prototype.updateFlow = function updateMovingFrontVFX() {
  ...
  const block = 5;
  const body = [], fronts = [], deposits = [], powder = [], spray = [];
  const regions = [];
  ...
  for (let bz = 1; bz < size - block; bz += block) {
    for (let bx = 1; bx < size - block; bx += block) {
      ...
      regions.push({ bx, bz, cx, cz, worldX, worldZ, y, dir, mass, dense, dep, pow });
    }
  }
  ...
  for (const r of regions) {
    ...
    const speed = Math.min(1, r.mass / 5);

    if (r.mass > .025) {
      const width = 2.6 + Math.min(5.2, Math.sqrt(r.mass) * 2.0);
      const length = 3.2 + Math.min(7.5, Math.sqrt(r.mass) * 2.7);
      push(body, r.worldX - r.dir.x * .9, r.y + .42, r.worldZ - r.dir.z * .9,
        width, .55 + speed * .28, length, angle);
      push(body, r.worldX + r.dir.x * 1.45, r.y + .48, r.worldZ + r.dir.z * 1.45,
        width * .86, .48 + speed * .25, length * .68, angle);
    }
    ...
    if (r.mass > .09) {
      ...
      if (downstreamMass < r.mass * .58) {
        ...
        push(fronts,
          r.worldX + r.dir.x * 2.25,
          r.y + .55 + height * .28,
          r.worldZ + r.dir.z * 2.25,
          width, height, 1.15 + speed * .55, angle + Math.PI / 2);
  ...
  for (let i = 0; i < Math.min(7, energeticFronts.length); i++) {
    ...
    const strength = Math.sqrt(r.mass + r.pow * 1.4);
    const cloud = 1.4 + Math.min(3.8, strength * 1.65);
    const side = (hash(r.bx, r.bz, 11) - .5) * r.width * .35;

    push(powder,
      r.worldX - r.dir.x * .4 + sideX * side,
      r.y + 1.5 + cloud * .62,
      r.worldZ - r.dir.z * .4 + sideZ * side,
      cloud * 1.3, cloud, cloud * 1.05, r.angle);

    const count = 10 + Math.floor(Math.min(16, strength * 5));
    for (let p = 0; p < count; p++) {
      const h1 = hash(r.bx + p * 5, r.bz - p * 3, 19);
      const h2 = hash(r.bx - p * 7, r.bz + p * 2, 23);
      const forward = .6 + h1 * 4.5;
      const lateral = (h2 - .5) * r.width * .9;
      const s = .06 + h1 * .13;
      push(spray,
...
function makeFront(name, color) {
  const mesh = BABYLON.MeshBuilder.CreateCapsule(name, {
    height: 3.0, radius: .82, tessellation: 18, capSubdivisions: 6
  }, this.scene);
  mesh.rotation.x = Math.PI / 2;
  mesh.bakeCurrentTransformIntoVertices();
  const material = this.mat(`${name}Mat`, color, .9);
  material.emissiveColor = color.scale(.07);
  mesh.material = material;
  mesh.isPickable = false;
  mesh.alwaysSelectAsActiveMesh = true;
  return mesh;
...
function makePowder() {
  ...
  material.emissiveColor = new BABYLON.Color3(.055, .045, .06);
  mesh.material = material;
  mesh.isPickable = false;
  mesh.alwaysSelectAsActiveMesh = true;
  return mesh;
